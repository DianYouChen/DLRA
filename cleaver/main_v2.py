...
from util.raw_data import load_nc, save_nc
from util.file_util import  get_latest, listAllFiles
from util.array_data import FixedSizeArray
...
class Cleaver:
    def __init__(self, inp_dir:str, oup_dir:str, cwd_dir:str, 
                 mask_fname:str, fixed_array_fname:str):
        ...
        self.name_list = listAllFiles(inp_dir)
    ...
    def check(self):      
        ...
        for targetfile in tqdm(self.name_list):            

            """
            check if the previous data exists
            """
            prev_time = self.get_time_from_path(targetfile) - timedelta(minutes=10)
            output_dt = self.get_time_from_path(targetfile) - timedelta(minutes=50)
            ...
            if (not os.path.exists(Cleaver.get_path_from_time(self.inp_dir, prev_time))) \
                ...
                data_increment = (load_nc(targetfile) - fix_sized_array.five_sum()) * mask_orig
                ...
                # 4. find data == 0
                mask_new = (load_nc(targetfile) == 0) * 1
                ...
                # 8. leave
                #print(f'{os.path.basename(targetfile)} has been downscaled.')
...
if __name__ == "__main__":
    inp_dir = "/bk2/vancechen/QPESUMS/yilin"
    oup_dir = "/bk2/vancechen/QPESUMS/raw/rain"
    cwd_dir = "/wk171/vancechen/cleaver"
    ...
    rain_cleaver = Cleaver(inp_dir, oup_dir, cwd_dir, mask_fname, fixed_array_fname)
    rain_cleaver.check()
...
if __name__ == "__main__":
    ...
    rain_cleaver = Cleaver(inp_dir, oup_dir, cwd_dir, mask_fname, fixed_array_fname)
    zero_value_file = "/bk2/vancechen/QPESUMS/yilin/2022/202202/20220215_0300.nc"
    zero_value = load_nc(zero_value_file)

    clever_name_list = listAllFiles(oup_dir)
    for fname in tqdm(sorted(clever_name_list)):

        next_time = rain_cleaver.get_time_from_path(fname) + timedelta(minutes=10)
        next_file = rain_cleaver.get_path_from_time(oup_dir, next_time)
        

        if not(os.path.isfile(next_file)):
            print(next_file)    
            save_nc(next_file, zero_value)
