...
class SimpleGRU(BaseConvRNN):
    ...
    def forward(self, 
        ...
        if is_PONI:
            if states is None:
                raise RuntimeError("Strange!!! No hidden state for forecaster input is not acceptable.")
            if inputs is None:
                raise RuntimeError("Strange!!! No inputs for forecaster is not acceptable.")
