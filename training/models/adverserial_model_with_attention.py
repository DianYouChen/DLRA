...
#         self.attention1 = TargetAttentionLean_1()
#         self.attention2 = TargetAttentionLean_2()
#         self.attention3 = TargetAttentionLean_3()

    def forward(self, input):
        pred = super().forward(input) # pred shape: [3, batch, H, W]
        # NOTE most recent rain frame
        output = self.attention(input[:, -1, -1:], pred) # input shape: [batch, Seq, C, H, W]
#         output = torch.clone(pred)
#         output[0] = self.attention1(input[:, -1:, -1], pred[0]) # input shape: [batch, Seq, C, H, W]
#         output2 = torch.clone(output)
#         output2[1] = self.attention2(output[0][:, None], output[1])
#         output3 = torch.clone(output2)
#         output3[2] = self.attention3(output2[1][:, None], output2[2])
        return output
