...
from models.adversarial_model_PONI import BalAdvPoniModel
from models.adversarial_model_PONI_persist import BalAdvPoniAttenModel
from models.AdvPONI_hetero_from_poni import PoniModel_addponi, PoniAttenModel_addponi
from models.AdvPONI_hetero_from_atten import PoniModel_addatten
...
from models.forecaster import (Forecaster, Forecaster_PONI, Forecaster_individual_weight_PONI, 
                               Forecaster_individual_weight_addPONI, Forecaster_addPONI)
...
def get_model(
    ...
    if model_type == ModelType.BaselineCNN:
        ...
        model = BaseLineModel(
            ...
            ModelType.BalancedGRUAdvPONI,
            ModelType.BalGRUAdvPONI_addponi,
            ModelType.BalancedGRUAdvPONIAtten,
            ModelType.BalGRUAdvPONIAtten_addponi,
            ModelType.BalGRUAdvPONI_addatten,
            ModelType.BalancedGRUAdvIndepenWeightPONI,
            ModelType.BalGRUAdvIndepenWeightPONI_addponi,
    ]:
        print(f'Using {ModelType.name(model_type)} model')
        # assert DataType.count1D(data_kwargs['data_type']) == 4 # "not necessary now"
        nc = DataType.count(data_kwargs['data_type'])
        if model_type in [
                ...
                ModelType.SSIMGRUModel, ModelType.BalancedGRUAdvPONI, ModelType.BalancedGRUAdvPONIAtten,
                ModelType.BalGRUAdvPONIAtten_addponi, ModelType.BalGRUAdvPONI_addponi, ModelType.BalGRUAdvPONI_addatten,
                ModelType.BalancedGRUAdvIndepenWeightPONI, ModelType.BalGRUAdvIndepenWeightPONI_addponi,
        ]:
            # 建立encoder和decoder
            if model_type in [ModelType.BalancedGRUAdvPONI, ModelType.BalancedGRUAdvPONIAtten,]:
                if model_kwargs['LCL']:
                    forecaster = Forecaster_PONI(forecaster_params_GRU[1],forecaster_params_GRU[2], data_kwargs['target_len'],
                                                 model_kwargs['teach_force'])
                else:
                    forecaster = Forecaster_PONI(forecaster_params_GRU[0],forecaster_params_GRU[2], data_kwargs['target_len'],
                                                 model_kwargs['teach_force'])
            elif model_type in [ModelType.BalancedGRUAdvIndepenWeightPONI,]:
                if model_kwargs['LCL']:
                    forecaster = Forecaster_individual_weight_PONI(forecaster_params_GRU[1],forecaster_params_GRU[2], data_kwargs['target_len'],
                                                 model_kwargs['teach_force'])
                else:
                    forecaster = Forecaster_individual_weight_PONI(forecaster_params_GRU[0],forecaster_params_GRU[2], data_kwargs['target_len'],
                                                 model_kwargs['teach_force'])
            elif model_type in [ModelType.BalGRUAdvPONI_addponi, ModelType.BalGRUAdvPONIAtten_addponi,]:
                nc_minus = DataType.countMinus(data_kwargs['data_type'])
                nc -= nc_minus
                if model_kwargs['LCL']:
                    forecaster = Forecaster_addPONI(forecaster_params_GRU[1],forecaster_params_GRU[2],
                                                    data_kwargs['target_len'],model_kwargs['teach_force'])
                else:
                    forecaster = Forecaster_addPONI(forecaster_params_GRU[0],forecaster_params_GRU[2],
                                                    data_kwargs['target_len'],model_kwargs['teach_force'])
            elif model_type in [ModelType.BalGRUAdvIndepenWeightPONI_addponi,]:
                nc_minus = DataType.countMinus(data_kwargs['data_type'])
                nc -= nc_minus
                if model_kwargs['LCL']:
                    forecaster = Forecaster_individual_weight_addPONI(forecaster_params_GRU[1],forecaster_params_GRU[2],
                                                    data_kwargs['target_len'],model_kwargs['teach_force'])
                else:
                    forecaster = Forecaster_individual_weight_addPONI(forecaster_params_GRU[0],forecaster_params_GRU[2],
                                                    data_kwargs['target_len'],model_kwargs['teach_force'])
            elif model_type in [ModelType.BalGRUAdvPONI_addatten,]:
                nc_minus = DataType.countMinus(data_kwargs['data_type'])
                nc -= nc_minus
                if model_kwargs['LCL']:
                    forecaster = Forecaster_PONI(forecaster_params_GRU[1],forecaster_params_GRU[2], data_kwargs['target_len'],
                ...
                if model_kwargs['LCL']:
                    forecaster = Forecaster(forecaster_params_GRU[1],forecaster_params_GRU[2], data_kwargs['target_len'])
                else:
                    forecaster = Forecaster(forecaster_params_GRU[0],forecaster_params_GRU[2], data_kwargs['target_len'])
        ...
        if data_kwargs['hetero_data'] == True:
            checkpoint_dict['hetr'] = 1
        ...
        if model_kwargs['teach_force'] != 0:
            checkpoint_dict['teaf'] = model_kwargs['teach_force']
        ...
        elif model_type == ModelType.BalancedGRUAdvPONI:
            # assert data_kwargs['data_type'] == DataType.Radar + DataType.Rain
        ...
        elif model_type == ModelType.BalancedGRUAdvIndepenWeightPONI:
            # assert data_kwargs['data_type'] == DataType.Radar + DataType.Rain
            model = BalAdvPoniModel(
                model_kwargs['adv_w'],
                model_kwargs['dis_d'],
        ...
        elif model_type == ModelType.BalancedGRUAdvPONIAtten:
            # assert data_kwargs['data_type'] == DataType.Radar + DataType.Rain
        ...
        elif model_type == ModelType.BalGRUAdvPONI_addponi:
        ...
        elif model_type == ModelType.BalGRUAdvIndepenWeightPONI_addponi:
            model = PoniModel_addponi(
                model_kwargs['adv_w'],
                model_kwargs['dis_d'],
                encoder,
        ...
        elif model_type == ModelType.BalGRUAdvPONIAtten_addponi:
        ...
        elif model_type == ModelType.BalGRUAdvPONI_addatten:
