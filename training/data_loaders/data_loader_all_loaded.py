...
def Altitude_data(keys):
        ...
        for key in keys:
            ...
            new_map, new_lat, new_lon = mapping(latList, lonList, target_map, (540, 420))
...
class DataLoaderAllLoaded(data.Dataset):
    ...
    def _set_index_map(self):
        ...
        test_sample_idx = np.random.randint(len(self._time)-2, size=5)
        ### for evaluating o'clock time ###
        self._time = self._time[1:]
        print("First and last time steps:", self._time[0], self._time[-1])
        print("Random time steps:", self._time[test_sample_idx[0]], self._time[test_sample_idx[2]], self._time[test_sample_idx[3]])
        ####################################
    ...
    def six_multiplication(self, input_data, factor=2000):
        ...
        output_map = np.stack(output_map, axis=0)/factor# [6, 540, 420]
        return output_map
    ...
    def get_era5_data(self, ec_dir, index, level, keys = ['u', 'v']):
        ...
        """
        # All TW region (added by Vance)
        crop_x, crop_y = self._img_size
        startx = 561 // 2 -  crop_x// 2
        starty = 441 // 2 - crop_y // 2
        lat = np.linspace(latStart,latEnd,561)[startx:startx + crop_x]
        lon = np.linspace(lonStart,lonEnd,441)[starty:starty + crop_y]
