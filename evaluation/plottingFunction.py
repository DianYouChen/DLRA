...
def HSS_line_plot(names, *data: list, numbers, colors=None, target_len=3) -> (None):
    # basic setting
    data_num = len(data)
    ...
    order = list(np.arange(0, target_len*data_num, target_len))
...
def CSI_line_plot(names, *data: list, numbers, colors=None, target_len=2) -> (None):
    # basic setting
    data_num = len(data)
    x_tick = np.array([1,3,5,10,15,20,30,40,60,90], dtype=np.int16)
    if colors == None:
        colors = ['C'+str(i) for i in range(data_num)]
    linestyles = ['-', '--', ':'] # target_length
    ...
    # another legend
    for t in range(target_len):
        none = ax2.plot(-41, 0, ls=linestyles[t], color='k',lw=4)
    ax2.legend(['0-1 hr', '0-3 hr', '2-3 hr'],loc='upper right',bbox_to_anchor=(1,0.95),
               fontsize=15,handlelength=2.5)
    
    hetero_exps = ['PONI_WTDOT', 'PONI_ThetaE', 'PONI_Season', 'PONI_Terrain', 'PONI_Wind']
    # main plot code
    for i in range(data_num): # how many different data
        for t in range(target_len): # target length
            if names[i] in hetero_exps:
                pt = ax.plot(range(len(x_tick)), data[i][t], color=colors[i], 
                            linestyle=linestyles[t], marker='o', lw=3, label=names[i], 
                            markersize=7.5, zorder=i,
                            ) # zorder只能用在同一個ax中
            else:
                pt = ax.plot(range(len(x_tick)), data[i][t], color=colors[i], 
                            linestyle=linestyles[t], marker='o', lw=4, label=names[i], 
                            markersize=10, zorder=(data_num-i),
                            ) # zorder只能用在同一個ax中
    ba = ax2.bar(np.arange(len(x_tick))+0.5, numbers, color='gray', alpha=0.5, width=0.5)
    ax.grid(axis='y', ls='--')
    ax.set_title('CSI scores')
    ...
    ax.set_xticks(np.arange(0,len(x_tick)))
    ax.set_xticklabels(x_tick, fontsize=16)
    ax.set_yticks(np.arange(1,6)/10)
    ax.set_yticklabels(np.arange(1,6)/10, fontsize=14)
...
def PFD_2D(preds: list, 
    ...
    '''
    This function is for single label plotting.
    ...
    * label size = [time, X, Y]
    ...
    for id, pred in enumerate(tqdm(preds, ncols=60)):
        ...
        for threshold in thresholds:
            ...
            ax.text(success_ratio, probability_detection, str(threshold),
...
def CDF_2D(preds: list, 
           label: np.array,
           thresholds: int = [10,20,30,40,50,70,100],  
           names: list = [],
           colors = None,
           ) -> (None):
    '''
    ...
    * label size = [time, X, Y]
    ...
    intvls = 10
    fig, ax = plt.subplots(1, 1, figsize=(10, 10), dpi=200, facecolor='w')
    for id, pred in enumerate(tqdm(preds, ncols=60)):
        assert pred.shape == label.shape, 'Size inconsistency.'
        all_pred = pred.reshape(-1)
        print("shape of all preds after reshaped:", all_pred.shape)
        all_pred.sort()
        print("shape of all preds after reshaped:", all_pred.shape)   
        raw_data_shape = all_pred.shape[0]

        new_idic = np.array([ K for K in range(0,raw_data_shape,intvls) ])
        new_all_pred = np.array([ all_pred[idx] for idx in new_idic ])
        print("shape of all new preds:", new_all_pred.shape)
        del all_pred   
        prbs = (np.arange(0, raw_data_shape,1)+1)/raw_data_shape
        print("shape of all prbs:", prbs.shape)
        new_prbs = np.array([ prbs[idx] for idx in new_idic ])
        print("shape of all new prbs:", new_prbs.shape)
        del new_idic, prbs
    
        pt = ax.plot(new_all_pred, new_prbs,
                     color=colors[id], linestyle=linestyles[0], lw=4, label=names[id], zorder=1
                    )
    del preds, pred

    all_label = label.reshape(-1)
    print("shape of all labels after reshaped:", all_label.shape)
    del label
    all_label.sort()
    print("shape of all labels after sorted:", all_label.shape)
    raw_data_shape = all_label.shape[0]

    new_idic      = np.array([ K for K in range(0,raw_data_shape,intvls) ])
    new_all_label = np.array([ all_label[idx] for idx in new_idic ])
    del all_label, new_idic


    ax.plot(new_all_label, new_prbs,
            color=colors[-1], linestyle=linestyles[0], lw=4, label='QPESUMS', zorder=2
            )
    del new_all_label, new_prbs

    ax.set_title('CDF')
    ax.set_xlabel('threshold (mm)')
    ax.set_ylabel('probability')
    ax.set_axisbelow(True)
    ax.set_xlim(-1,x_tick[-1])
    ax.set_xticks(np.append(np.array([0]), x_tick))
    ax.set_ylim(0,1.1)
...
def boxPlot(groups: list,  
    ...
    * group size = [N models][M thresholds][array]
    ...
    if colors == None:
        colors = ['C' + str(i) for i in range(groupNum)]
    ### plot
    ...
    if groupNum % 2 == 1:
        ...
        for id, group in enumerate(tqdm(groups, ncols = 60)):
            ...
            legend_handles.append(ax.boxplot(group,
                                    ...
                                    whis = [5, 95],
                                    widths = boxWidth,
                                    boxprops = dict(facecolor = colors[id], alpha = 0.7),
    ...
    if '130' in tickNames[-1]:
        ax.set_ylim(bottom=-150) # 2-3 hr
